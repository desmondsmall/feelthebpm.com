...
if (!process.env.GETSONGBPM_API_KEY) {
  console.error('GETSONGBPM_API_KEY is not set. Run: GETSONGBPM_API_KEY=your_key node pipeline/tools/eval-gsb.mjs');
  process.exit(1);
}
...
W.push('');
W.push(`Generated by \`pipeline/tools/eval-gsb.mjs\`. **${conflicts.length}** songs where Deezer and GSB disagree by >3 BPM`);
...
W.push('You know these songs — add the right tempo to `pipeline/inputs/bpm_overrides.json` to resolve each. Nothing is dropped meanwhile.');
W.push('');
